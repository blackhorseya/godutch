...
import (
	...
	"github.com/blackhorseya/godutch/internal/app/godutch/biz/user/mocks"
...
func (s *handlerSuite) Test_impl_Signup() {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{email: email1, password: pass1, name: name1, mock: func() {
				s.mock.On("Signup", mock.Anything, email1, pass1, name1).Return(nil, er.ErrSignup).Once()
			}},
			wantCode: 500,
		},
		{
			...
			args: args{email: email1, password: pass1, name: name1, mock: func() {
				s.mock.On("Signup", mock.Anything, email1, pass1, name1).Return(info1, nil).Once()
			}},
			wantCode: 201,
		},
	}
...
func (s *handlerSuite) Test_impl_Logout() {
	s.r.DELETE("/api/v1/auth/logout", s.handler.Logout)
	...
	tests := []struct {
		...
		{
			name: "logout then error",
			args: args{mock: func() {
				s.mock.On("Logout", mock.Anything).Return(er.ErrUpdateToken).Once()
			}},
			wantCode: 500,
		},
		{
			name: "logout then success",
			args: args{mock: func() {
				s.mock.On("Logout", mock.Anything).Return(nil).Once()
			}},
			wantCode: 200,
		},
	}
	for _, tt := range tests {
		s.T().Run(tt.name, func(t *testing.T) {
			...
			uri := fmt.Sprintf("/api/v1/auth/logout")
