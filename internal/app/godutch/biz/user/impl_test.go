...
func (s *bizSuite) Test_impl_Signup() {
	...
	for _, tt := range tests {
		s.T().Run(tt.name, func(t *testing.T) {
			...
			gotInfo, err := s.biz.Signup(tt.args.ctx, tt.args.email, tt.args.password, tt.args.name)
			if (err != nil) != tt.wantErr {
...
func (s *bizSuite) Test_impl_Logout() {
	type args struct {
		ctx  contextx.Contextx
		mock func()
	}
	...
	for _, tt := range tests {
		s.T().Run(tt.name, func(t *testing.T) {
			...
			if err := s.biz.Logout(tt.args.ctx); (err != nil) != tt.wantErr {
